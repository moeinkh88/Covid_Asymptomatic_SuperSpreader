...
@model function fitprob(data, prob)
	...
	ρ₁ ~ truncated(Normal(0,1); lower=0, upper=1-ρ₂)
	...
	δₐ~ truncated(Normal(0,1); lower=0, upper=10)
...
posterior_samples = sample(chain[[:β, :β´´,:NN, :κ,:l, :β′, :ρ₁,:γₐ, :γᵢ, :γᵣ,:δᵢ ,:δₚ , :δₕ, :δₐ]], nChain; replace=false)

Err=zeros(nChain)
for i in 1:nChain
	pp=Array(posterior_samples.value[:,:,1])[i,:]
	β, β´´, NN, κ, l, β′, ρ₁,γₐ,γᵢ,γᵣ,δᵢ,δₚ , δₕ,  δₐ = pp[1:14]
	p = [10280000/NN, β, l, β′, β´´, κ, ρ₁,	ρ₂,	γₐ,	γᵢ,	γᵣ,	δᵢ,	δₚ, δₕ, δₐ]
	S0=10280000/NN-5; E0=0; I0=4; P0=1; A0=0; H0=0; R0=0; F0=0
