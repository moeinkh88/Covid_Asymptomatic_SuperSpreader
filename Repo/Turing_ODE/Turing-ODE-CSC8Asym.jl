...
# Similar to Turin-ODE-CSC7Asym but remove NN 
...
@model function fitprob(data, prob)
	...
	p=[N, β, l, β′, β´´, κ, ρ₁,	ρ₂,	γₐ,	γᵢ,	γᵣ,	δᵢ,	δₚ, δₕ, δₐ]
...
posterior_samples = sample(chain[[:β, :β´´, :κ,:l, :β′, :ρ₁, :ρ₂, :γₐ, :γᵢ, :γᵣ,:δᵢ ,:δₚ , :δₕ, :δₐ]], nChain; replace=false)

Err=zeros(nChain)
for i in 1:nChain
	pp=Array(posterior_samples.value[:,:,1])[i,:]
	β, β´´, κ, l, β′, ρ₁, ρ₂, γₐ,γᵢ,γᵣ,δᵢ,δₚ , δₕ,  δₐ = pp[1:14]
	p = [N, β, l, β′, β´´, κ, ρ₁,	ρ₂,	γₐ,	γᵢ,	γᵣ,	δᵢ,	δₚ, δₕ, δₐ]
	S0=N-5; E0=0; I0=4; P0=1; A0=0; H0=0; R0=0; F0=0
