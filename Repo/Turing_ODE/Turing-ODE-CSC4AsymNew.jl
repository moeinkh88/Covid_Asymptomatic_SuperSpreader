...
Confirmed=dataset_CC[dataset_CC[!,2].=="India",45:351] #comulative confirmed data of Portugal from 3/2/20 to 5/17/20
...
DeathData=dataset_D[dataset_D[!,2].=="India",45:351]
TrueF=diff(Float64.(Vector(DeathData[1,:])))

dataset_R = CSV.read("time_series_covid19_recovered_global.csv", DataFrame) # all data of Recover
RData=dataset_R[dataset_R[!,2].=="India",45:351]
...
# Initial conditions
E0=0; I0=0; P0=0; A0=3; H0=0; R0=0; F0=0
...
X0=[N-3, E0, I0, P0, A0, H0, R0, F0] # initial values
...
@model function fitprob(data, prob)
	...
	β´´~ truncated(Normal(1, 10); lower=.1, upper=10)
    NN ~ truncated(Normal(1, 2000); lower=1, upper=5000)
	κ ~ truncated(Normal(0, 4); lower=0, upper=4)
	l ~ truncated(Normal(0, 1); lower=0, upper=1)
	β′ ~ truncated(Normal(0, 2); lower=0, upper=2)
	ρ₁ ~ truncated(Normal(0,1); lower=0, upper=1)
	ρ₂ ~ truncated(Normal(0,1); lower=0, upper=1)
	...
	δₐ~ truncated(Normal(0,1); lower=0, upper=1)

	p=[10280000/NN, β, l, β′, β´´, κ, ρ₁,	ρ₂,	γₐ,	γᵢ,	γᵣ,	δᵢ,	δₚ, δₕ, δₐ]
	S0=10280000/NN-3; E0=0; I0=0; P0=0; A0=3; H0=0; R0=0; F0=0
...
nChain=8000
chain = sample(model, NUTS(0.65), MCMCSerial(), nChain, 5; progress=false)
