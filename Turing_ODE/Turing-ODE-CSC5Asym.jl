...
Confirmed=dataset_CC[dataset_CC[!,2].=="Spain",38:142] #confirmed of spain from 2/24/20 to 6/7/20
C=diff(Float64.(Vector(Confirmed[1,:])))# Daily new confirmed cases

#preporcessing (map negative values to zero)
₋Ind=findall(C.<0)
C[₋Ind].=0.0
## System definition
...
# Initial conditions
E0=0; I0=10; P0=1; A0=0; H0=0; R0=0; F0=0
N=47000000/425 # Population Size
...
X0=[N-11, E0, I0, P0, A0, H0, R0, F0] # initial values
...
@model function fitprob(data, prob)
	...
	β´´~ truncated(Normal(1, 10); lower=.1, upper=50)
	...
	δₐ~ truncated(Normal(0,1); lower=0, upper=50)

	p=[47000000/NN, β, l, β′, β´´, κ, ρ₁,	ρ₂,	γₐ,	γᵢ,	γᵣ,	δᵢ,	δₚ, δₕ, δₐ]
	S0=47000000/NN-11; E0=0; I0=10; P0=1; A0=0; H0=0; R0=0; F0=0
	...
	# Observations
	for i in 1:length(IPH)
	data[i] ~ Normal(IPH[i], σ^2)
	end

    return nothing
end

model = fitprob(C, prob)

# Sample 3 independent chains with forward-mode automatic differentiation (the default).
nChain=5000
chain = sample(model, NUTS(0.65), MCMCSerial(), nChain, 3; progress=false)

posterior_samples = sample(chain[[:β, :β´´,:NN, :κ,:l, :β′, :γₐ, :γᵢ, :γᵣ,:δᵢ ,:δₚ , :δₕ, :δₐ]], nChain; replace=false)

Err=zeros(nChain)
for i in 1:nChain
	pp=Array(posterior_samples.value[:,:,1])[i,:]
	β, β´´, NN, κ, l, β′, γₐ,γᵢ,γᵣ,δᵢ,δₚ , δₕ,  δₐ = pp[1:13]
	p = [47000000/NN, β, l, β′, β´´, κ, ρ₁,	ρ₂,	γₐ,	γᵢ,	γᵣ,	δᵢ,	δₚ, δₕ, δₐ]
	S0=47000000/NN-11; E0=0; I0=10; P0=1; A0=0; H0=0; R0=0; F0=0
